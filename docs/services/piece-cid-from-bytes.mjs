...
function create(code7, digest3) {
  ...
  return new Digest(code7, size2, digest3, bytes);
}
function decode3(multihash) {
  const bytes = coerce(multihash);
  const [code7, sizeOffset] = decode2(bytes);
  const [size2, digestOffset] = decode2(bytes.subarray(sizeOffset));
...
function from({ name: name5, prefix: prefix2, encode: encode5, decode: decode7 }) {
...
function decode4(string, alphabetIdx, bitsPerChar, name5) {
...
function rfc4648({ name: name5, prefix: prefix2, bitsPerChar, alphabet }) {
  ...
  return from({
    ...
    decode(input) {
      return decode4(input, alphabetIdx, bitsPerChar, name5);
    }
  });
}
...
var empty2 = () => EMPTY;
var EMPTY = from2(new Uint8Array(NODE_SIZE).fill(0));
...
var Hasher = class {
  ...
  digestInto(output, byteOffset = 0, asMultihash = true) {
    ...
    if (asMultihash) {
      varint_exports.encodeTo(code6, output, endOffset);
      endOffset += TAG_SIZE;
      const size2 = paddingLength + HEIGHT_SIZE + ROOT_SIZE;
      const sizeLength = varint_exports.encodingLength(size2);
...
var fromDigest = (digest3) => fromLink(create2(code5, digest3));
var fromLink = (link) => {
  if (link.code !== code5) {
    throw new TypeError(`Piece link must have raw encoding`);
  }
  if (link.multihash.code !== code6) {
    throw new Error(`Piece link must have ${name4} multihash`);
  }
  return new Piece(link);
};
var fromPayload = (payload) => fromDigest(digest2(payload));
var toLink = (piece) => create2(code5, fromPiece2(piece));
var Piece = class {
  /**
...
// piece-cid-entry.mjs
function computePieceCidFromBytes(bytes) {
  const p = fromPayload(bytes);
  return toLink(p);
}
export {
  computePieceCidFromBytes
};
